...
if ( ! class_exists( 'MOQC_Shop_Min_Max_Step' ) ) {
	/**
	 * Class MOQC_Shop_Min_Max_Step.
	 */
	class MOQC_Shop_Min_Max_Step {
			...
			if ( $product ) {
				...
				if ( get_option( 'enable_fields' ) === 'yes' && ! empty( get_option( 'select_products' ) ) || ! empty( get_option( 'select_categories' ) ) ) {
					...
					foreach ( get_option( 'select_products' ) as $key => $selected_prods ) {
						if ( strval( $selected_prods ) === strval( $product->id ) ) {
				...
				} elseif ( strval( get_option( 'enable_fields' ) ) === 'yes' && empty( get_option( 'select_products' ) ) && empty( get_option( 'select_categories' ) ) ) {
					$prod_quantity = get_option( 'minimum_qty' );
				}
